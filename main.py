...
# Railway and Cloud Hosts ke liye clean and standard Bybit domain
...
# ==========================================
# FETCH OHLCV (FIXED FOR BYBIT V5 RESPONSE)
# ==========================================
def fetch_ohlcv(symbol):
    ...
    try:
        ...
        # Bybit returns data from newest to oldest. 
        # Pandas requires oldest to newest for correct EMA calculations.
        raw.reverse() 

        # Bybit V5 elements: [startTime, openPrice, highPrice, lowPrice, closePrice, volume, turnover]
        df = pd.DataFrame(
            raw,
            columns=["ts", "open", "high", "low", "close", "volume", "turnover"]
        )

        # Explicitly converting text/objects to float numbers
...
def detect_signal(df):
    ...
    # Safely extracting indicators for current and previous candles
    current_above = ema20.iloc[-1] > ema200.iloc[-1]
    previous_above = ema20.iloc[-2] > ema200.iloc[-2]

    # Crossover condition check
...
async def run_bot():
    ...
    # Context Manager integration for safe asynchronous connections
    async with bot:
        try:
        ...
        while True:
            ...
            # Scanning top 300 active liquid pairs
            for symbol in PAIRS[:300]:
                ...
                # Rate limiting sleep to prevent Bybit/Telegram IP ban
...
if __name__ == "__main__":
    if "APNA" in BOT_TOKEN or not BOT_TOKEN:
        log.error("Set valid BOT_TOKEN in environment variables first")
        raise SystemExit

    if "APNA" in CHAT_ID or not CHAT_ID:
        log.error("Set valid CHAT_ID in environment variables first")
        raise SystemExit
